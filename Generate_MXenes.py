...
def generate_MXene(configuration_type, num_MX_atoms, dissertation_avg_M_X_bonds_df, dissertation_avg_M_Tx_bonds_df, dissertation_avg_Tx_Tx_bonds_df, x_frac_list, y_frac_list, Tx_x_frac_list, Tx_y_frac_list, theta_MX_list, theta_MTx_list, phi_MX_list, phi_MTx_list, atom_list, desired_layer_distance, num_Tx, Tx_atoms_list):
    ...
    for i in range(len(atom_list) - 1):
        if num_Tx == 2 and (i == 0 or i == len(atom_list) - 2):  # do not want to base the cell on the Tx-Tx bond length
            if Tx_atoms_list[0] == 'H' and Tx_atoms_list[1] in ['F','Cl','Br','I']:
                new_bond = dissertation_avg_Tx_Tx_bonds_df.loc[Tx_atoms_list[0], Tx_atoms_list[1]]
            else:
                new_bond = dissertation_avg_Tx_Tx_bonds_df.loc[Tx_atoms_list[1], Tx_atoms_list[0]]
        elif (num_Tx == 2 and i == 1) or (num_Tx == 1 and i == 0):  # bottom M-Tx bond
            new_bond = dissertation_avg_M_Tx_bonds_df.loc[atom_list[i + 1], atom_list[i]]
            ...
            new_bond = dissertation_avg_M_Tx_bonds_df.loc[atom_list[i], atom_list[i + 1]]
            if not ('t' in configuration_type) and new_bond < min_bond_len:
                min_bond_len = new_bond
                longest_bond_pair_indexes = i
        elif (i-num_Tx)%2 == 0: # M element
            new_bond = dissertation_avg_M_X_bonds_df.loc[atom_list[i], atom_list[i + 1]]
            if new_bond < min_bond_len:
                min_bond_len = new_bond
                longest_bond_pair_indexes = i
        else: # X element
            new_bond = dissertation_avg_M_X_bonds_df.loc[atom_list[i + 1], atom_list[i]]
            if new_bond < min_bond_len:
                min_bond_len = new_bond
                longest_bond_pair_indexes = i
            ...
            if i == 0:  # bottommost Tx atom
                ...
                if num_Tx == 2:
                    if atom_list[i] == 'H' and atom_list[i + 1] in ['F','Cl','Br','I']:
                        new_bond = dissertation_avg_Tx_Tx_bonds_df.loc[atom_list[i], atom_list[i + 1]]
                    else:
                        new_bond = dissertation_avg_Tx_Tx_bonds_df.loc[atom_list[i + 1], atom_list[i]]
...
def get_avg_bond_dfs(directory, avg_M_X_bonds_name, avg_M_Tx_bonds_name, avg_Tx_Tx_bonds_name, n_value):
    ...
    dissertation_avg_M_X_bonds_df = pd.read_csv(directory + avg_M_X_bonds_filename)
    column_names = dissertation_avg_M_X_bonds_df.columns.tolist()
    dissertation_avg_M_X_bonds_df = dissertation_avg_M_X_bonds_df.set_index(column_names[0])

    dissertation_avg_M_Tx_bonds_df = pd.read_csv(directory + avg_M_Tx_bonds_filename)
    column_names = dissertation_avg_M_Tx_bonds_df.columns.tolist()
    dissertation_avg_M_Tx_bonds_df = dissertation_avg_M_Tx_bonds_df.set_index(column_names[0])

    dissertation_avg_Tx_Tx_bonds_df = pd.read_csv(directory + avg_Tx_Tx_bonds_filename)
    column_names = dissertation_avg_Tx_Tx_bonds_df.columns.tolist()
    dissertation_avg_Tx_Tx_bonds_df = dissertation_avg_Tx_Tx_bonds_df.set_index(column_names[0])

    return dissertation_avg_M_X_bonds_df, dissertation_avg_M_Tx_bonds_df, dissertation_avg_Tx_Tx_bonds_df
...
avg_Tx_Tx_bonds_name = 'dissertation_avg_Tx_Tx_bonds'

desired_layer_distance = 15



dissertation_avg_M_X_bonds_df, dissertation_avg_M_Tx_bonds_df, dissertation_avg_Tx_Tx_bonds_df = get_avg_bond_dfs(bond_directory, avg_M_X_bonds_name, avg_M_Tx_bonds_name, avg_Tx_Tx_bonds_name, n_value)
